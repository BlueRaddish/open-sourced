...
describe('Open SourcED app', () => {
  ...
  it('changes and persists appearance settings', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Neural read-aloud' })).toBeInTheDocument()
    ...
    expect(screen.getByText(/playback requests full app volume/i)).toBeInTheDocument()
  ...
  it('requires explicit confirmation before a paid provider generation', async () => {
    sessionStorage.setItem('open-source-ed.openrouter.session-key', 'visitor-key')
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => String(input).includes('/api/v1/models')
      ? new Response(JSON.stringify({ data: [{ id: 'openai/gpt-test', name: 'GPT Test', created: 1, context_length: 100_000, pricing: { prompt: '0.000001', completion: '0.000002' }, supported_parameters: ['structured_outputs'], architecture: { output_modalities: ['text'] } }] }), { status: 200, headers: { 'Content-Type': 'application/json' } })
      : new Response(JSON.stringify({ configured: false }), { status: 200, headers: { 'Content-Type': 'application/json' } })))
    render(<App />)
    fireEvent.click(screen.getByRole('button', { name: 'Generate' }))
    fireEvent.click(screen.getByRole('button', { name: /use GPT models/i }))
    expect(await screen.findByRole('option', { name: 'GPT Test' })).toBeInTheDocument()
    expect(screen.getByText(/your account, your cost/i)).toBeInTheDocument()
    const generate = screen.getByRole('button', { name: /generate editable cards/i })
    expect(generate).toBeDisabled()
    fireEvent.click(screen.getByLabelText(/allow this one generation/i))
