...
export function SharedSetView({ set, error, save, browse }: Props) {
  ...
  return <section className="page-width page-section shared-page">
    ...
    <div className="set-banner" style={{ '--set-color': set.color } as React.CSSProperties}><div><span>{set.subject}</span><h1>{set.title}</h1><p>{set.description || 'A shared Open SourceED study set.'}</p><small>{set.cards.length} cards</small></div><BookOpen className="shared-book" /></div>
