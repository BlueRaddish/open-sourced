...
export function MockTest({ set, back, finish }: Props) {
  const questions = useMemo(() => shuffle(set.cards).slice(0, Math.min(20, set.cards.length)).map((card) => ({ card, choices: choicesFor(card, set.cards) })), [set])
