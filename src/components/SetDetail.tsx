...
export function SetDetail({ set, progress, navigate, edit, duplicate, toggleArchive, remove, share }: Props) {
  ...
  return <section className="page-width page-section">
    ...
    <div className="term-list">{set.cards.map((card) => <article key={card.id}><div><h3>{card.term}</h3><p>{card.definition}</p>{card.note && <small>{card.note}</small>}</div><div className="term-actions"><ReadAloudButton compact text={[card.term, card.definition, card.note]} label={`Read ${card.term} aloud`} /><span className={`mastery-badge level-${Math.floor(mastery(progress[card.id]) / 34)}`}>{mastery(progress[card.id])}%</span></div></article>)}</div>
  </section>
}
