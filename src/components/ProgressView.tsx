...
export function ProgressView({ sets, progress, attempts, activityDates, backup, restore, reset, openSet }: Props) {
  ...
  return <section className="page-width page-section">
    ...
    <div className="data-panel"><div><h2>Your data, under your control</h2><p>Open SourceED stores the library and proficiency history locally in this browser. Export a backup before clearing browser data or moving devices.</p></div><button className="danger-outline" onClick={reset}><RotateCcw size={17} /> Reset all local data</button></div>
  </section>
}
