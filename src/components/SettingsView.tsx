...
export function SettingsView({ preferences, update, backup }: Props) {
  return <section className="page-width page-section settings-page">
    ...
    <div className="settings-layout"><div className="settings-main">
      ...
      <article className="settings-card audio-check"><div className="settings-title"><Volume2 /><div><h2>Neural read-aloud</h2><p>Open SourcED now uses the open-source Piper Plus multilingual voice by default instead of relying on voices installed on your device.</p></div></div><div className="audio-test-row"><div><b>One-time voice download</b><span>The first use downloads about 40 MB for the model plus the speech engine, then caches them in this browser. Speech is generated locally with no account, API key, or usage credits. If loading fails, Open SourcED falls back to a device voice.</span><span>Playback requests full app volume; your browser tab, operating system, and output device still control the final audible level.</span></div><div><ReadAloudButton text="Open SourcED read aloud is working." label="Test English voice" /><ReadAloudButton text={[{ text: '日本語の音声テストです。', lang: 'ja-JP', rate: .8 }]} label="Test Japanese voice" /></div></div></article>
